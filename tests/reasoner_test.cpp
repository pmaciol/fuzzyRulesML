...
TEST(SimpleReasoning, add_double_rule) {
  ...
  const auto result = reasoner.do_reasoning(fru::RuleTestingValues(
      {{fru::FuzzyVarUnion{petal_length}, fru::CrispValuesUnion{0.0}}, {fru::FuzzyVarUnion{petal_width}, fru::CrispValuesUnion{0.0}}}));
  EXPECT_EQ(result.size(), 1);
  EXPECT_EQ(result.at({"iris_type", "Setosa"}), 1.0);

  const auto result2 = reasoner.do_reasoning(fru::RuleTestingValues(
      {{fru::FuzzyVarUnion{petal_length}, fru::CrispValuesUnion{1.0}}, {fru::FuzzyVarUnion{petal_width}, fru::CrispValuesUnion{1.0}}}));
  EXPECT_EQ(result2.size(), 2);
  EXPECT_GT(result2.at({"iris_type", "Setosa"}), result2.at({"iris_type", "Versicolor"}));
  const auto result3 = reasoner.do_reasoning(fru::RuleTestingValues(
      {{fru::FuzzyVarUnion{petal_length}, fru::CrispValuesUnion{1.0}}, {fru::FuzzyVarUnion{petal_width}, fru::CrispValuesUnion{2.0}}}));

  EXPECT_EQ(result3.size(), 2);
  EXPECT_LT(result3.at({"iris_type", "Setosa"}), result3.at({"iris_type", "Versicolor"}));
  const auto result4 = reasoner.do_reasoning(fru::RuleTestingValues(
      {{fru::FuzzyVarUnion{petal_length}, fru::CrispValuesUnion{3.0}}, {fru::FuzzyVarUnion{petal_width}, fru::CrispValuesUnion{3.0}}}));
