...
using Membership = std::map<std::size_t, double>;
...
inline auto make_linear_distribution(const initial_distribution::Uniform<UNDERLYING_TYPE>& distribution) {
  ...
  const auto section_length = static_cast<UNDERLYING_TYPE>(distribution.get_max() - distribution.get_min()) / (distribution.get_categories() - 1);
  ...
  const auto right = fuzzyrulesml::mfunct::LinearMemberFunctAsc(distribution.get_max() - static_cast<UNDERLYING_TYPE>(section_length), distribution.get_max());
  ...
  for (std::size_t i = 0; i < number_of_internal_functions; i++) {
    functions.push_back(fuzzyrulesml::mfunct::LinearMemberFunct{distribution.get_min() + static_cast<UNDERLYING_TYPE>(i * section_length),
...
template <> inline auto FuzzyVariable<int>::operator()(const UnderlyingType&) const -> FuzzyValue<int> { return FuzzyValue<int>({}); }
