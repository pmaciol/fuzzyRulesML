...
auto RulesSet::add_output_variable(std::string_view variable_name, std::vector<std::string> output_categories) -> std::vector<std::string> {
...
void RulesSet::add_rule(const std::map<FuzzyVarUnion, std::size_t>& variables_map, const Conclusion& conclusion) {
  std::ranges::all_of(variables_map | std::views::keys, [this](const auto& variable) {
    return input_variables.contains(variable);
  });
  // TODO: "Check if the variables are already in the input_variables";
  // TODO: "Check if the variables are already in the conclusions";
...
// ?????
// auto get_rules(const std::map<FuzzyVarUnion, CrispValuesUnion>& variables_map) -> std::multimap<FuzzyVarUnion, std::size_t> {
//   std::multimap<FuzzyVarUnion, std::size_t> output_map;
//   for (const auto& variable : variables_map) {
//     const Membership memberships =
//         std::visit([val = variable.second](const auto& var) { return var.operator()(val).get_membership(); }, variable.first);
